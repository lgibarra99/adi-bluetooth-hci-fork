...
If you're actively developing MAX-BLE-HCI or want to modify the source code, you should install the package in **editable/development mode**. This allows you to make changes to the source code and have them immediately available without reinstalling.

### Quick Installation

**Important**: If you're using a virtual environment (like `.venv`), make sure to activate it first before installing:

```bash
# Activate your virtual environment first (if using one)
source .venv/bin/activate

# Then run the installation script from the repository root
cd /path/to/MAX-BLE-HCI
./install_dev.sh
```
...
**Note**: The package must be installed in each Python environment where you want to use it (global Python or each virtual environment separately).

### Benefits of Development Mode

- ✅ Changes to source code in `src/max_ble_hci/` are immediately active
- ✅ No need to reinstall after making modifications
- ✅ Package is importable from anywhere: `from max_ble_hci import BleHci`
- ✅ Works with all example scripts in the `examples/` directory

### Verify Installation

After installing in development mode, verify it works:

```bash
python3 -c "from max_ble_hci import BleHci; print('Success!')"
```

### Development Workflow

1. Install in editable mode once: `pip install -e .`
2. Make changes to source code in `src/max_ble_hci/`
3. Run scripts immediately - no reinstall needed
4. Test your changes

### Uninstalling Development Mode

To uninstall the development installation:

```bash
pip uninstall max_ble_hci
```

Then reinstall from PyPi if needed:
